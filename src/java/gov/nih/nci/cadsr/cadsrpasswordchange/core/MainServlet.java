	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			if (servletPath.equals(Constants.SERVLET_URI + "/login")) {
				...
				if (session != null) {
					logger.debug("MainServlet.doPost non-null session");					
					session.invalidate();
				}				
...
//			}
	...
	throws ServletException, Exception {
		...
		try {
			...
			if(doValidateAttemptedCount(session, req, resp, Constants.ASK_USERID_URL) == false) {
				return;
			}			
...
//			String status = (String)arr.get(PasswordChangeDAO.ACCOUNT_STATUS);
	...
	//CADSRPASSW-42
	private boolean doValidateAttemptedCount(HttpSession session,HttpServletRequest req, HttpServletResponse resp, String redictedUrl) throws Exception {
		boolean retVal = true;
	...
	throws Exception {
		...
		if(doValidateAttemptedCount(session, req, resp, "./jsp/askQuestion1.jsp") == false) {
			return;
		}
	...
	throws Exception {
		...
		if(doValidateAttemptedCount(session, req, resp, "./jsp/askQuestion2.jsp") == false) {
			return;
		}
	...
	throws Exception {
		...
		if(doValidateAttemptedCount(session, req, resp, "./jsp/askQuestion3.jsp") == false) {
			return;
		}
